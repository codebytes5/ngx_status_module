...
ngx_mds_stats_handler(ngx_http_request_t *r) 
{
	...
	// html_body_start
	len = snprintf(0, 0, html_body_start);
	if (len<0)
    	return NGX_HTTP_INTERNAL_SERVER_ERROR;
    b = ngx_create_temp_buf(r->pool, len);
    if (b == NULL)
        return NGX_HTTP_INTERNAL_SERVER_ERROR;
	b->last = ngx_sprintf(b->last, html_body_start);
	if (add_ngx_buf(&cl, &ll, b, r))
		return NGX_HTTP_INTERNAL_SERVER_ERROR;
	len_tot += len;
	...
	// html_body_end
	len = snprintf(0, 0, html_body_end);
	if (len<0)
    	return NGX_HTTP_INTERNAL_SERVER_ERROR;
    b = ngx_create_temp_buf(r->pool, len);
    if (b == NULL)
        return NGX_HTTP_INTERNAL_SERVER_ERROR;
	b->last = ngx_sprintf(b->last, html_body_end);
	if (add_ngx_buf(&cl, &ll, b, r))
		return NGX_HTTP_INTERNAL_SERVER_ERROR;
	len_tot += len;
	
	//
	len = snprintf(0, 0, str3);
	if (len<0)
    	return NGX_HTTP_INTERNAL_SERVER_ERROR;
	b = ngx_create_temp_buf(r->pool, len);
	if (b == NULL)
		return NGX_HTTP_INTERNAL_SERVER_ERROR;
	b->last = ngx_sprintf(b->last, str3);
	if (add_ngx_buf(&cl, &ll, b, r))
			return NGX_HTTP_INTERNAL_SERVER_ERROR;
	len_tot += len;
	
	// html_footer
	len = snprintf(0, 0, html_footer);
	if (len<0)
    	return NGX_HTTP_INTERNAL_SERVER_ERROR;
    b = ngx_create_temp_buf(r->pool, len);
    if (b == NULL)
